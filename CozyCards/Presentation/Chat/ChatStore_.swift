...
/// thread and forwards persistence to `ChatRepository`/`ChatTranscriptStoring_`,
/// mirroring how `LibraryStore` wraps `LibraryRepository`.
///
/// There's always a `currentThread` — the one `ChatView_` renders, together
...
@Observable
@MainActor
final class ChatStore_ {


    private(set) var threads: [ChatThread] = []
    private(set) var currentThread: ChatThread

    private let repository: any ChatRepository & ChatTranscriptStoring_
    private var session: LanguageModelSession


    init(repository: any ChatRepository & ChatTranscriptStoring_) {
    ...
    func send(prompt: String) async {
        ...
        do {
            for try await partial in session.streamResponse(to: prompt) {
                message.answer = .text(partial.content)
            }
