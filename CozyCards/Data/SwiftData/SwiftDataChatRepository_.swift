...
/// domain model.
protocol ChatTranscriptStoring_: Sendable {
...
/// `SwiftDataLibraryRepository_`. `ChatThread`/`ChatMessage` are plain
...
/// by `@MainActor`.
@MainActor
final class SwiftDataChatRepository_: ChatRepository, ChatTranscriptStoring_, @unchecked Sendable {
    ...
    func threads() -> [ChatThread] {
        let descriptor = FetchDescriptor<ChatThreadModel_>(
    ...
    func add(_ thread: ChatThread) {
        modelContext.insert(ChatThreadModel_(thread: thread))
        save()
        broadcast()
    }
    ...
    func save(_ thread: ChatThread) {
        ...
        row.messages = thread.messages.map { ChatMessageModel_(message: $0) }
    ...
    private func fetchRow(id: UUID) -> ChatThreadModel_? {
        var descriptor = FetchDescriptor<ChatThreadModel_>(
    ...
    private func save() {
        do {
            ...
            print("SwiftDataChatRepository_ save failed:", error)
        }
    }
