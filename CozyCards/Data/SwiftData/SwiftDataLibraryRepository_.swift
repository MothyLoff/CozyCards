...
@ModelActor
actor SwiftDataLibraryRepository_: LibraryRepository {
    ...
    func all() -> [LibraryItem] {
        let descriptor = FetchDescriptor<LibraryItemModel_>(
    ...
    func add(_ item: LibraryItem) {
        modelContext.insert(LibraryItemModel_(item: item))
        save()
        broadcast()
    }
    ...
    private func fetchRow(id: UUID) -> LibraryItemModel_? {
        var descriptor = FetchDescriptor<LibraryItemModel_>(
    ...
    private func save() {
        do {
            ...
            print("SwiftDataLibraryRepository_ save failed:", error)
        }
    }
