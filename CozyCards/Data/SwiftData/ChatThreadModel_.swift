...
/// On-disk row for a `ChatThread`, owning its `ChatMessageModel_` rows.
@Model
final class ChatThreadModel_ {
    ...
    @Relationship(deleteRule: .cascade, inverse: \ChatMessageModel_.thread)
    var messages: [ChatMessageModel_] = []
    ...
    /// is written and read directly by `SwiftDataChatRepository_`.
